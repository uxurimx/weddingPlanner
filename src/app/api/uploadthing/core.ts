  ...
  photographerUpload: f({ image: { maxFileSize: '16MB', maxFileCount: 200 } })
    ...
    .onUploadComplete(async ({ metadata, file }) => {
      try {
        ...
        console.error('[photographerUpload onUploadComplete]', e)
        throw e
      }
    }),
  ...
  guestUpload: f({
    ...
    .onUploadComplete(async ({ metadata, file }) => {
      try {
        ...
        console.error('[guestUpload onUploadComplete]', e)
        throw e
      }
    }),
  ...
  videoMessage: f({ video: { maxFileSize: '256MB', maxFileCount: 1 } })
    ...
    .onUploadComplete(async ({ metadata, file }) => {
      try {
        ...
        await db
          .update(invitations)
          .set({ videoMessageUploaded: true })
          .where(eq(invitations.id, metadata.invitationId))
        await logNotification({
          eventId:      metadata.eventId,
          invitationId: metadata.invitationId,
          type:         'video',
          message:      'Un invitado dejó un video mensaje privado para los novios',
        })
      } catch (e) {
        console.error('[videoMessage onUploadComplete]', e)
        throw e
      }
    }),
